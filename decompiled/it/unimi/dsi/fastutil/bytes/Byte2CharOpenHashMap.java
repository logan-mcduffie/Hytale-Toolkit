         ...
         if (!(o instanceof Entry)) {
            ...
            if (e.getKey() == null || !(e.getKey() instanceof Byte)) {
               ...
               if (k == 0) {
                  ...
                  byte curr;
                  int pos;
                  if ((curr = key[pos = HashCommon.mix((int)k) & Byte2CharOpenHashMap.this.mask]) == 0) {
                     return false;
                  } else if (k == curr) {
      ...
      public int nextEntry() {
         if (!this.hasNext()) {
            ...
            if (this.mustReturnNullKey) {
               ...
               int p = HashCommon.mix((int)k) & Byte2CharOpenHashMap.this.mask;
      ...
      private void shiftKeys(int pos) {
         ...
         while (true) {
            ...
            for (pos = pos + 1 & Byte2CharOpenHashMap.this.mask; (curr = key[pos]) != 0; pos = pos + 1 & Byte2CharOpenHashMap.this.mask) {
               int slot = HashCommon.mix((int)curr) & Byte2CharOpenHashMap.this.mask;
               if (last <= pos ? last >= slot || slot > pos : last >= slot && slot > pos) {
                  if (pos < last) {
   ...
   private abstract class MapSpliterator<ConsumerType, SplitType extends Byte2CharOpenHashMap.MapSpliterator<ConsumerType, SplitType>> {
      ...
      public boolean tryAdvance(ConsumerType action) {
         if (this.mustReturnNull) {
            ...
            for (byte[] key = Byte2CharOpenHashMap.this.key; this.pos < this.max; this.pos++) {
               if (key[this.pos] != 0) {
                  this.c++;
      ...
      public long estimateSize() {
         return !this.hasSplit
            ...
            : Math.min(
               (long)(Byte2CharOpenHashMap.this.size - this.c),
