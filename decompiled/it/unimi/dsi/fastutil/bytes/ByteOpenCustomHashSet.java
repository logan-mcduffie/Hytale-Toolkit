   ...
   private final class SetSpliterator implements ByteSpliterator {
      ...
      public boolean tryAdvance(ByteConsumer action) {
         if (this.mustReturnNull) {
            ...
            for (byte[] key = ByteOpenCustomHashSet.this.key; this.pos < this.max; this.pos++) {
               if (key[this.pos] != 0) {
                  this.c++;
      ...
      public long estimateSize() {
         return !this.hasSplit
            ...
            : Math.min(
               (long)(ByteOpenCustomHashSet.this.size - this.c),
      ...
      public ByteOpenCustomHashSet.SetSpliterator trySplit() {
         if (this.pos >= this.max - 1) {
            ...
            if (retLen <= 1) {
               ...
               int retPos = this.pos;
               ByteOpenCustomHashSet.SetSpliterator split = ByteOpenCustomHashSet.this.new SetSpliterator(retPos, myNewPos, this.mustReturnNull, true);
               this.pos = myNewPos;
               this.mustReturnNull = false;
               this.hasSplit = true;
               return split;
