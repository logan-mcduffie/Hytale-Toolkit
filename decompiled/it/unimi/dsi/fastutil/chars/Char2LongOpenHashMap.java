         ...
         if (!(o instanceof Entry)) {
            ...
            if (e.getKey() == null || !(e.getKey() instanceof Character)) {
               ...
               if (k == 0) {
                  ...
                  char curr;
                  int pos;
                  if ((curr = key[pos = HashCommon.mix((int)k) & Char2LongOpenHashMap.this.mask]) == 0) {
                     return false;
                  } else if (k == curr) {
      ...
      public int nextEntry() {
         if (!this.hasNext()) {
            ...
            if (this.mustReturnNullKey) {
               ...
               int p = HashCommon.mix((int)k) & Char2LongOpenHashMap.this.mask;
      ...
      private void shiftKeys(int pos) {
         ...
         while (true) {
            ...
            for (pos = pos + 1 & Char2LongOpenHashMap.this.mask; (curr = key[pos]) != 0; pos = pos + 1 & Char2LongOpenHashMap.this.mask) {
               int slot = HashCommon.mix((int)curr) & Char2LongOpenHashMap.this.mask;
               if (last <= pos ? last >= slot || slot > pos : last >= slot && slot > pos) {
                  if (pos < last) {
   ...
   private abstract class MapSpliterator<ConsumerType, SplitType extends Char2LongOpenHashMap.MapSpliterator<ConsumerType, SplitType>> {
      ...
      public boolean tryAdvance(ConsumerType action) {
         if (this.mustReturnNull) {
            ...
            for (char[] key = Char2LongOpenHashMap.this.key; this.pos < this.max; this.pos++) {
               if (key[this.pos] != 0) {
                  this.c++;
      ...
      public long estimateSize() {
         return !this.hasSplit
            ...
            : Math.min(
               (long)(Char2LongOpenHashMap.this.size - this.c),
