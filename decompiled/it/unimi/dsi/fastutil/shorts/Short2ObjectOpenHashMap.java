         ...
         if (!(o instanceof Entry)) {
            ...
            if (e.getKey() != null && e.getKey() instanceof Short) {
               ...
               if (k == 0) {
                  ...
                  short curr;
                  int pos;
                  if ((curr = key[pos = HashCommon.mix((int)k) & Short2ObjectOpenHashMap.this.mask]) == 0) {
                     return false;
                  } else if (k == curr) {
      ...
      public int nextEntry() {
         if (!this.hasNext()) {
            ...
            if (this.mustReturnNullKey) {
               ...
               int p = HashCommon.mix((int)k) & Short2ObjectOpenHashMap.this.mask;
      ...
      private void shiftKeys(int pos) {
         ...
         while (true) {
            ...
            for (pos = pos + 1 & Short2ObjectOpenHashMap.this.mask; (curr = key[pos]) != 0; pos = pos + 1 & Short2ObjectOpenHashMap.this.mask) {
               int slot = HashCommon.mix((int)curr) & Short2ObjectOpenHashMap.this.mask;
               if (last <= pos ? last >= slot || slot > pos : last >= slot && slot > pos) {
                  if (pos < last) {
   ...
   private abstract class MapSpliterator<ConsumerType, SplitType extends Short2ObjectOpenHashMap<V>.MapSpliterator<ConsumerType, SplitType>> {
      ...
      public boolean tryAdvance(ConsumerType action) {
         if (this.mustReturnNull) {
            ...
            for (short[] key = Short2ObjectOpenHashMap.this.key; this.pos < this.max; this.pos++) {
               if (key[this.pos] != 0) {
                  this.c++;
      ...
      public long estimateSize() {
         return !this.hasSplit
            ...
            : Math.min(
               (long)(Short2ObjectOpenHashMap.this.size - this.c),
