...
public final class DoubleArrays {
   ...
   public static double[] grow(double[] array, int length, int preserve) {
      if (length > array.length) {
         int newLength = (int)Math.max(Math.min((long)array.length + (array.length >> 1), 2147483639L), (long)length);
         ...
         while (j-- != 0) {
            executorCompletionService.submit(() -> {
               ...
               while (true) {
                  ...
                  while (ixxx-- != first) {
                     count[(int)(fixDouble(a[ixxx]) >>> shift & 255L ^ signMask)]++;
                  }
         ...
         while (j-- != 0) {
            executorCompletionService.submit(() -> {
               ...
               while (true) {
                  ...
                  while (ixx-- != first) {
                     count[(int)(fixDouble(a[perm[ixx]]) >>> shift & 255L ^ signMask)]++;
                  }
      ...
      if (to - from < 1024) {
         ...
         while (stackPos > 0) {
            ...
            while (ixx <= ix) {
               double t = a[ixx];
               double u = b[ixx];
               c = (int)(fixDouble(k[ixx]) >>> shift & 255L ^ signMask);
               if (ixx < ix) {
                  int d;
                  while ((d = --pos[c]) > ixx) {
            ...
            while (j-- != 0) {
               executorCompletionService.submit(() -> {
                  ...
                  while (true) {
                     ...
                     while (ixxxx-- != first) {
                        count[(int)(fixDouble(k[ixxxx]) >>> shift & 255L ^ signMask)]++;
                     }
