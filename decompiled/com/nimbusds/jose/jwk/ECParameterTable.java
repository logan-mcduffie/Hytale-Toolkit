...
public class ECParameterTable {
   ...
   private static final ECParameterSpec SECP256K1_SPEC = new ECParameterSpec(
      new EllipticCurve(
   ...
   private static final ECParameterSpec P_384_SPEC = new ECParameterSpec(
      new EllipticCurve(
   ...
   private static final ECParameterSpec P_521_SPEC = new ECParameterSpec(
      new EllipticCurve(
         new ECFieldFp(
            new BigInteger(
   ...
   public static ECParameterSpec get(Curve curve) {
      if (Curve.P_256.equals(curve)) {
         ...
         return Curve.P_521.equals(curve) ? P_521_SPEC : null;
      }
   }
   ...
   public static Curve get(ECParameterSpec spec) {
      if (spec == null) {
         return null;
      } else if (spec.getCurve().getField().getFieldSize() == P_256_SPEC.getCurve().getField().getFieldSize()
         ...
         return spec.getCurve().getField().getFieldSize() == P_521_SPEC.getCurve().getField().getFieldSize()
