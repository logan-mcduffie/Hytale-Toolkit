...
public class GlobalTrafficShapingHandler extends AbstractTrafficShapingHandler {
   private final ConcurrentMap<Integer, GlobalTrafficShapingHandler.PerChannel> channelQueues = new ConcurrentHashMap<>();
   ...
   private GlobalTrafficShapingHandler.PerChannel getOrSetPerChannel(ChannelHandlerContext ctx) {
      ...
      GlobalTrafficShapingHandler.PerChannel perChannel = this.channelQueues.get(key);
      if (perChannel == null) {
   ...
   public void handlerRemoved(ChannelHandlerContext ctx) throws Exception {
      Channel channel = ctx.channel();
      Integer key = channel.hashCode();
      GlobalTrafficShapingHandler.PerChannel perChannel = this.channelQueues.remove(key);
      if (perChannel != null) {
   ...
   private void sendAllValid(ChannelHandlerContext ctx, GlobalTrafficShapingHandler.PerChannel perChannel, long now) {
      synchronized (perChannel) {
         for (GlobalTrafficShapingHandler.ToSend newToSend = perChannel.messagesQueue.pollFirst();
            newToSend != null;
